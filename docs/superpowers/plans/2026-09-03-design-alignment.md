...
- Modify: `src/FlowSure.tsx` — delete `btnGhost`/`changeBtn` style constants if still present (both were already removed as a side effect of Task 11's rewrite — verify with grep rather than assuming). **Do NOT delete `demoRow`** — still actively used by the Onboarding screen (Task 5), only its definition was updated there, never its usage. **Note:** `btnPrimary` and `iconBtn` were already deleted in Task 6 (both became fully unused once the Confirm screen — their last consumer — was converted); if you find any of these four already gone, that's expected, not a gap to re-fix.
...
**Do NOT delete `demoRow`** — it is still actively used by the Onboarding screen's demo-location list (`style={demoRow}` on each demo-location button, landed in Task 5). It was never meant to be removed, only its definition was updated in Task 5 (border dropped). Deleting it now would break the Onboarding screen.

Delete `btnGhost` and `changeBtn` if still present (`FlowSure.tsx:1861-1927` in the original file) — check first with `grep -n "btnGhost\|changeBtn" src/FlowSure.tsx`, since both may already be gone: `btnPrimary`/`iconBtn` were deleted in Task 6, and Task 11's own self-review reported removing `btnGhost`/`changeBtn` too (both became fully unused once Task 11 converted their last remaining call sites). If your grep shows zero hits for `btnGhost`/`changeBtn`, that's expected — confirm they're absent rather than trying to re-delete them, and don't treat their absence as a gap in your own work.
